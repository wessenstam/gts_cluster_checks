...
def checks_to_run(server_ip,user,passwd):
    ...
    if "4" not in str(json_search):
        print("The Blueprints have not been uploaded...")

    # Do we have images uploaded?
    ...
    json_search = get_json_data(server_ip_pc, url, payload, method, user, passwd, value)
    if int(json_search['metadata']['total_matches']) < 27:
        print("The images have not been uploaded...")
        print("Checking which image is not at the PC...")
        images = ("ERA-Server-build-1.2.0.1.qcow2", "CentOS7.qcow2", "Windows2016.qcow2", "Linux_ToolsVM.qcow2",
        ...
        for image_check in sorted(images):
            if image_check not in str(json_search):
